...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        print(hyp)
        model = Model(cfg="models/yolov5s.yaml",ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    ...
    if pretrained:
        ...
        if ckpt['optimizer'] is not None:
            ...
            pass        # 最初保存的模型没有训练到300epoch而提前终止了，导致这个结构不对了，目前不需要resume
            """
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #imgs = imgs.to(device, non_blocking=True).float() / 255.0*2-1  # uint8 to float32, 0-255 to 0.0-1.0
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--weights', type=str, default='yolov5svoc.pt', help='initial weights path')
    ...
    parser.add_argument('--epochs', type=int, default=300)#206
